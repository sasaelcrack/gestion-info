# service.py
# Módulo de lógica de negocio — CRUD completo conectado a persistencia JSON
# Módulo 3: funciones renombradas + list comprehensions + lambda

from validate import validar_registro
from file import load_data, save_data

_ventas: list = load_data()
_ids_registrados: set = {v["id"] for v in _ventas}
...
def new_register(id_venta: str, cliente: str, producto: str,
...
def list_records(orden: str = "id") -> list:
    return sorted(_ventas, key=lambda v: v.get(orden, ""))

def search_record(id_venta: str) -> dict | None:
    coincidencias = [v for v in _ventas if v["id"] == id_venta.strip()]
    return coincidencias[0] if coincidencias else None

def update_record(id_venta: str, **kwargs) -> tuple[bool, str]:
...
def delete_record(id_venta: str) -> tuple[bool, str]:
    global _ventas

    if search_record(id_venta) is None:
        return False, f"Error: no se encontró la venta con ID '{id_venta}'."
